...
0. **站点的 macOS 拦截是站长定的，别绕**：Cloudflare 那份部署（`bobbychina-games.pages.dev`）在 `functions/_middleware.js` 里对 macOS **服务端直接 403**（连游戏资源都不下发）；主域 GitHub Pages 那一份没有服务端，由 `/mac-block.js` 客户端兜底。你要是在 Mac 上打开游戏发现打不开，那是预期行为不是 bug —— 调试请用 Windows / Android，或另找一台机器。
...
- **当前版本（2026-09-23）**：Boss 二版已上线 —— 血量基础 **4000**（5:00 实到 ≈9800）、四档七招（新增激光横扫 / 落石）、
  **护盾阶段**（破 62% / 30% 时无敌 + 召唤 8~10 只小弟，清光才破盾，破盾后 2.5s 受伤 ×1.35）、18% 以下狂暴；
  ...
  TTK 实测：中期构筑（bolt Lv5 + 光环 Lv4、零被动、满帧）**121 DPS → 81 秒**；满构筑约 3~5 倍 DPS → 20~30 秒。
- **量 Boss 血量前必读（三个坑都在秤上，不在游戏里）**：① 标签页不在前台 → `requestAnimationFrame` 被节流，
  伤害按帧结算 → 帧率减半 DPS 也减半（用 CDP `Emulation.setFocusEmulationEnabled` 打开焦点模拟，并把帧率一起量出来）；
  ② 卡片六个乘区（伤害/攻速/范围/弹数/幸运/暴击）**跨局残留**，只重置伤害攻速时同一套构筑实测差一倍（160 vs 337 DPS）；
  ③ 打 Boss 期间还在升级自动选卡 → 60 秒后 DPS 自己从 120 跳到 200+，测量时必须冻结升级。
  三条修完，同一套构筑的 DPS 才稳定（121 / 122，逐 10 秒打点全程恒定）。
