...
(function (VS) {
  ...
  C.ENEMY_TYPES = {
    ...
    /* Boss 本体（加强版）：血量 3200 → 5400（**这是基础值**，实际还要乘难度曲线 HP_CURVE：
       5:00 首次登场时 ×2.45 ≈ 13230）。速度略降、撞击更疼 —— 打得久才谈得上"手法"。
       血量口径是量出来的，不是拍的：机器人中期构筑（bolt Lv5 + 光环 Lv4）对 Boss 实测
       **148~203 DPS**（差在贴不贴脸打），13230 血 → TTK **65~89 秒**（探针 ⑩ 的窗口是 20~95 秒）；
       9600 基础值那版实测打满 120 秒还杀不掉，而 120 血的角色在弹幕里活不到那么久，等于"打不动"。
       后续 Boss 再乘 HP_GROWTH 1.45。
       它的伤害减免由 shield 阶段与虚弱期共同决定（见 bosskit） */
    boss: {
      id: 'boss', name: '尸潮之王',
      hp: 5400, speed: 46, radius: 46, damage: 52, xp: 620,
