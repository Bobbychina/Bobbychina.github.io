  ...
  C.LEVELS = [
    ...
    },
    {
      ...
      bosses: [
        ...
        {
          /* 最终形态的血量（口径写死在这儿，改数字前先看这段）：
             实际 hp = lemonPig 基础 11000 × HP_CURVE@720s(3.60) × 本关 hpMul(1.30) × 这里的 hpMul
             2026-09-25 站长点单"削弱到 3.8 万"：
               原来 hpMul 1.6 → 实测 **82,368**（11000 × 3.6 × 1.3 × 1.6）
               目标 38000 → hpMul = 38000 / (11000 × 3.6 × 1.3) = 0.73815068…
               取 **0.7381507**：实测 37,999.99，HUD 走 Math.round + 千分位 → **显示 38 000 / 38 000**。
               （试过 0.738 显示 37 992、0.7382 显示 38 003，都差一点，所以取到小数点后七位。）
             注意 HP_CURVE 的关键帧在 600s 就封顶（curve() 超出末帧保持），
             所以 720s 用的就是 ×3.60，不会继续涨。
             只改这一条 —— 10:00 那只普通柠檬猪（走 BOSS.SCHEDULE，该条目没有 hpMul）
             用的是 lemonPig 基础 11000 × 3.6 × 1.3 = 51,480，**保持原样未动**。 */
          at: 720, type: 'lemonPig', name: '柠檬猪 · 最终形态', hpMul: 0.7381507,
